...
15. **LanceDB `table_names()` → `list_tables()` is NOT a drop-in replacement.** In LanceDB 0.27.1, `list_tables()` returns a `ListTablesResponse` Pydantic object, not `list[str]`. The `in` operator silently returns `False` on this object (it iterates Pydantic field tuples, not table names). Use the `_table_names(db)` helper in `src/storage.py` which extracts `.tables` from the response. See `research/list-tables-regression.md` for the full post-mortem.
...
37. **Never apply API migrations without testing the actual return value.** Deprecation warnings describe intent, not behavior. A sub-agent replaced `table_names()` → `list_tables()` based solely on the deprecation message, assuming identical return types. The actual return type was a Pydantic object, not a list. The `in` operator silently returned wrong results (no error, no warning). **Rule: always run `type(result)` and `repr(result)` in a REPL before assuming API equivalence.** This applies to any library upgrade or deprecated-API replacement. See `research/list-tables-regression.md`.
...
43. **Pipeline self-notifies via `clawdbot system event`.** `run_episode.sh` uses `clawdbot system event --mode now` for completion notifications (success or failure). ERR trap catches failures with line number and exit code. This replaces the old approach of having the cron agent monitor logs — which caused session context bloat. The script is fire-and-forget: cron agent starts it, confirms PID, exits. Script notifies when done.

44. **Cron session isolation is already built-in but prompt design matters.** The DTFHN cron job uses `sessionTarget: "isolated"` which gives a fresh session each run. But if the prompt tells the agent to "monitor logs until DONE appears," the agent stays alive for 30+ min, accumulating output. Fix: make the prompt fire-and-forget. Let the script handle notifications. Cron agent should exit in <30 seconds.

45. **Cloudflare Pages deploy hooks for site rebuild.** After R2 upload, `run_episode.sh` curls a deploy hook URL to trigger a Cloudflare Pages rebuild (so new episodes appear on pod.c457.org). The hook URL is stored in `.env` as `CF_PAGES_DEPLOY_HOOK_URL`. To create the hook: Cloudflare Dashboard → Pages → dailytechfeedsite → Settings → Builds & deployments → Deploy hooks → Add deploy hook. Paste the generated URL into `.env`.

46. **Podcast transcripts use Podcasting 2.0 `<podcast:transcript>` tag.** Namespace: `https://podcastindex.org/namespace/1.0`. The existing pipeline already generates `transcript.vtt` files with timing and `<v George Carlin>` speaker tags — no separate transcript generation step needed. VTT files are uploaded to `dtfhn/transcripts/DTFHN-{date}.vtt` on R2. The feed includes `<podcast:transcript url="..." type="text/vtt" language="en" />` per episode item. Apple Podcasts, Podcast Index, and other Podcasting 2.0 clients pick these up automatically. No changes to `run_episode.sh` needed — `upload_to_r2.py` handles transcript upload alongside the MP3.

47. **CLI was renamed: `clawdbot` → `openclaw`.** As of 2026-01-31, the system CLI is `openclaw`, not `clawdbot`. Any scripts using `clawdbot system event` must use `openclaw system event`. The command is at `/opt/homebrew/bin/openclaw`. This caused silent notification failures on 2026-01-31 (pipeline completed but no success notification was sent).
...
58. **Title-only articles can cause LLM preamble leakage — now fixed.** When an article has no content (fetch_status="title_only"), the LLM sometimes generates chain-of-thought preamble like "Now I have the full context. Let me write the script..." before the actual content. Fixed by `sanitize_llm_output()` which runs on ALL generated content (scripts, intros, outros, interstitials) before disk write. It strips leading preamble, `---` separators used as preamble boundaries, trailing meta-commentary, and markdown artifacts. Conservative by design — won't strip content starting with quotes, years, or questions. 33 tests in `tests/test_sanitize.py`.

59a. **Sanitize at the source, not at the sink.** `sanitize_llm_output()` is called inside each generation function (`generate_script`, `generate_interstitial`, `generate_intro`, `generate_outro`) rather than at the pipeline level where scripts are written to disk. This ensures sanitization happens regardless of how the function is called — from the pipeline, from tests, or from ad-hoc scripts. Defense in depth: intro/outro still have their own `_strip_preamble()` + `_strip_markdown()` for static-prefix enforcement on top of the general sanitizer.

59. **Host naming convention: Jack Hacksman (male voices), Jackie Hackswoman (female voices).** Male TTS voices (forbin, george_carlin, stephen_fry, philip_fry, noel, lynch) use "Jack Hacksman" as the on-air host identity. Female TTS voices (scarjo) use "Jackie Hackswoman." These are the podcast host names — NEVER use the real person's name as the host. The voice inspiration is credited in the outro via `VOICE_CREDITS`. Character docs live in `characters/` for new voices (STEPHEN_FRY.md, PHILIP_FRY.md, NOEL.md, SCARJO.md, LYNCH.md); legacy docs (FORBIN.md, CARLIN.md, GC.md) remain in the project root.

60_old. **Jack Hacksman is the host (default as of 2026-02-01).** The podcast host identifies as Jack Hacksman, NOT Dr. Forbin. The Forbin voice style (from "Colossus: The Forbin Project", 1970) is the voice *inspiration* — analytical, precise, sardonic. The on-air identity is "your host, Jack Hacksman." End-of-episode credit line attributes the voice inspiration. TTS voice: "forbin" on quato. Character file: `FORBIN.md`. Old characters preserved: `CARLIN.md`, `GC.md`.

60. **Segment MP3 archival replaces WAV-only intermediate pipeline.** As of 2026-02-02, each TTS WAV is immediately transcoded to a 192k CBR MP3 in `{episode_dir}/segments/`, validated (size > 1KB + ffprobe duration check), then the WAV is deleted. The "what needs rendering" check looks at `segments/*.mp3` — NOT `wav_temp/*.wav`. The final episode MP3 is stitched directly from segment MP3s via ffmpeg concat demuxer (`-c copy`). This means: (a) interrupted runs resume from segment MP3s, (b) segment audio is archived alongside text, (c) re-stitching doesn't require re-rendering. MP3 concat with `-c copy` adds ~0.12s of padding per stitch point due to MP3 frame alignment — acceptable for podcast audio. Key functions: `transcode_segment_to_mp3()`, `validate_segment_mp3()`, `stitch_mp3s_variable()` in `src/audio.py`. Both `generate_episode_audio.py` and `generate_missing_wavs.py` use segment MP3s as the source of truth.

62. **Resilient pipeline with LanceDB state tracking.** `pipeline_state` table tracks 5-phase pipeline progress per episode: fetch → scripts → tts → assembly → upload → complete. Each phase checks DB + disk before doing work. `run_episode.py` replaces the inline Python in `run_episode.sh` (shell script is now a thin wrapper). Key design: phases are idempotent — re-running skips completed work. TTS phase delegates to `generate_episode_audio.py` which already had segment MP3 resume logic. Upload phase tracks sub-steps independently (R2, site deploy, Telegram) so a Telegram failure doesn't re-upload. Status command: `python3 scripts/episode_status.py [date]`.

63. **Date-only episode format (YYYY-MM-DD) prevents directory sprawl.** Old format `YYYY-MM-DD-HHMM` created multiple directories per day on re-runs. New format uses one canonical dir per date. Old HHMM episodes are untouched in DB. `format_date_for_tts()` already handles both formats.
...
69. **Homograph disambiguation via LLM in TTS pipeline.** `src/homographs.py` uses claude-haiku-3-5 (via OpenClaw gateway) to disambiguate homographs — words spelled the same but pronounced differently based on context (read/reed/red, live/lyve/liv, record/REH-cord/reh-CORD, etc.). Dictionary in `data/homographs.json` (25+ entries). Runs inside `prepare_text_for_tts()` as Step 1, before regex pronunciation fixes. Fail-safe: returns original text on LLM failure or suspicious output length. Published scripts are NEVER modified — disambiguation only affects text sent to TTS. To add new homographs: add entries to `data/homographs.json` with variant keys and phonetic respelling values. The LLM picks the right variant based on sentence context.
